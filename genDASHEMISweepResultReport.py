...
def main() :
    ...
    parser.add_argument("-d", "--dir", help="absolute path to the directory that stores the result files ending with .clstPV", action="store", required=True)
    parser.add_argument("-p", "--prog", help="specify the program that produces these results", action="store", required=True)
    ...
    for dirName, subdirList, fileList in os.walk(args.dir):
      for x in fileList:
        if (x.endswith(".clstPV"):
          foundFile=True    
          processResultfile(os.path.join(dirName,x), args.prog)
          # then write out report
          writereport(args.dir, args.prog, 0, 23)
    if foundFile is not True:
       print "No result file."          

# write report
def writereport(workDir, usedProgram, vminColorBar, vmaxColorBar) :  
    ...
    if usedProgram.lower().startswith("chat"):
        plotResult(pltx_sPos,plty,caseCt,usedProgram,"Fisher's exact test PVs",vminColorBar,vmaxColorBar,38673387,39399147)#the dataset chat uses follows hg18/build36
    else:
        lineSeg(pltx_sPos,pltx_ePos,plty,caseCt,usedProgram,"Fisher's exact test PVs",True,vminColorBar,vmaxColorBar,40387120,41112880)#the dataset dash/emi uses follows hg19/build37

    
# Draw scatter plot with jitter (each clique is represented by the start marker of the locating bin)
def plotResult(x,y,caseCt,usedProgram,plotTitle,vminColorBar,vmaxColorBar,regionStart,regionEnd) :
...
def lineSeg(xmin,xmax,y,caseCt,usedProgram,plotTitle,isBPPos,vminColorBar,vmaxColorBar,regionStart,regionEnd) :
    ...
    if isBPPos :
      ...
      plt.savefig(repID+"_" + modelName+"_l_xid_" + str(plotXmin) + "_" + str(plotXmax) + ".png", format='png')
    plt.clf()   
